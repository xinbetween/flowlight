...
final class IPOwnerLookup: @unchecked Sendable {
    ...
    /// Traffic that stays on this Mac or the local network. The test itself lives in `NetworkScope`, which the
    /// filter extension can see; this stays as the name the rest of the app already calls.
    static func isLocalNetwork(_ ip: String) -> Bool { NetworkScope.isLocalNetwork(ip) }
